...
//
// Если вы не хотите отдельно обрабатывать эту ошибку, то можно установить ее
// значение в nil. Так же иногда бывает удобно переопределить эту ошибку на
// какую-то другую, более полезную в приложении. Например, для веб-сервера
// можно переопределить ее в rest.ErrNotFound и тогда не потребуется отдельной
// проверки на то, что значения с таким ключем нет в хранилище.
...
// Т.е. более короткие ключи имеют больший приоритет. Порядок сортировки
